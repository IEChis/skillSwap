...
export default function MatchCard({ match, onView, cta = '交换看看 →' }: Props) {
  ...
  return (
    <div className="animate-rise card match-card relative flex flex-col p-5">
      ...
      <StarDoodle className="match-star absolute -top-3 right-9 !h-7 !w-7 text-[#f2734e]" aria-hidden />
      <StarDoodle
        className="match-star match-star-sm absolute top-0.5 right-5 !h-4 !w-4 text-[#b9aaf7]"
        aria-hidden
      />
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate font-bold text-[#211c16]">{u.name}</h3>
            <span className="shrink-0 text-xs text-[#9a9082]">{u.city}</span>
          </div>
          <p className="mt-0.5 line-clamp-1 text-xs text-[#5c5446]">{u.bio}</p>
        </div>
        {bidirectional ? (
          <span className="badge shrink-0 border border-[#e2dafb] bg-[#f1edfd] text-[#5739c4]">
            <SwapIcon width={12} height={12} /> 双向匹配
          </span>
        ) : (
          <span className="badge shrink-0 border border-[#ece6dc] bg-[#fbf7f0] text-[#9a9082]">可以交换</span>
        )}
      </div>

      {/* 交换关系：卡片主角 */}
      <div className="match-panel mt-4 rounded-2xl bg-[#fbf7f0] p-4">
        {/* TA 想学 ←→ 你可以教 */}
        {teachRows.map((s) => (
          <div key={'t' + s} className="pair-x flex items-center justify-between gap-2 py-1">
            <div className="flex min-w-0 items-center gap-1.5">
              <span className="shrink-0 text-xs text-[#9a9082]">她想学</span>
            ...
            <div className="flex min-w-0 items-center justify-end gap-1.5">
              <span className="ex-teach min-w-0 truncate !px-2.5 !py-1 text-sm">你可以教 {s}</span>
            </div>
          </div>
        ))}
        ...
        {teachRows.length > 0 && learnRows.length > 0 && (
          <div className="flex flex-col items-center py-0.5">
            <span className="h-2.5 border-l-2 border-dotted border-[#c9bcf8]" />
            <SwapIcon width={15} height={15} className="match-swap my-0.5 text-[#6a4fe0]" />
            <span className="h-2.5 border-l-2 border-dotted border-[#f3b7a3]" />
          </div>
        )}
        ...
        {learnRows.map((s) => (
          <div key={'l' + s} className="pair-x flex items-center justify-between gap-2 py-1">
            <div className="flex min-w-0 items-center gap-1.5">
              <span className="shrink-0 text-xs text-[#9a9082]">她可以教</span>
            ...
            <div className="flex min-w-0 items-center justify-end gap-1.5">
              <span className="ex-learn min-w-0 truncate !px-2.5 !py-1 text-sm">你想学 {s}</span>
            </div>
          </div>
        ))}

        {teachRows.length + learnRows.length < match.iTeachThem.length + match.iLearnFromThem.length && (
          <p className="mt-1 text-center text-[11px] text-[#9a9082]">还有更多能换的…</p>
        )}
      </div>

      {/* 底部：匹配度放次要位置，主 CTA 在右侧 */}
      <div className="mt-4 flex items-center justify-between">
        <span className="text-xs text-[#9a9082]">
